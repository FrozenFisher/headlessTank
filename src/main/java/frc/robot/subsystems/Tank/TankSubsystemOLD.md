...
public class TankSubsystemOLD extends SubsystemBase{
    ...
    public TankSubsystemOLD(){
        if(Robot.isReal()){
            io = new TankIOPhoenixOLD();
        }
        else{
        ...
        turnPIDController.enableContinuousInput(-180.0, 180.0);//Input Range
        turnPIDController.setTolerance(2.0); //容差
        
        odometry = new DifferentialDriveOdometry(
            Rotation2d.fromDegrees(inputs.headingDegrees),
    ...
    private double calculateTurnOutput() {
        double output = turnPIDController.calculate(getHeading(), targetHeading);
    ...
    public void resetOdometry(Pose2d pose){
        odometry.resetPosition(Rotation2d.fromDegrees(inputs.headingDegrees), inputs.leftPositionMeters, inputs.rightPositionMeters, pose);
        currentPose = pose;
    }
    ...
    public void headlessMove(double speed) {
        speed = MathUtil.clamp(speed, 0.0, TankConstants.K_MAX_SPEED_MPS);
    ...
    public boolean headlessTurn(double targetAngle, boolean isLeftStickInput) {
        ...
        if (isLeftStickInput) {
            ...
            double forwardAngleDiff = MathUtil.inputModulus(targetAngle - currentHeading, -180.0, 180.0);
            double backwardAngleDiff = MathUtil.inputModulus((targetAngle + 180.0) - currentHeading, -180.0, 180.0);
            ...
            if (Math.abs(forwardAngleDiff) <= Math.abs(backwardAngleDiff)) {
                ...
                finalTargetAngle = MathUtil.inputModulus(targetAngle + 180.0, -180.0, 180.0);
